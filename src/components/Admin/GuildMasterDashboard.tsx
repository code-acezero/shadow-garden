      ...
      <div className="h-screen bg-[#050505] text-white font-sans flex flex-col md:flex-row relative overflow-hidden">
        ...
        <main className="flex-1 min-w-0 h-full md:h-screen overflow-y-auto p-4 md:p-8 relative z-10">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 min-h-[500px]">
            ...
            {activeTab === 'ANALYTICS' && <AnalyticsSection accentColor="red" />}
            ...
            {activeTab === 'PALETTES' && <ThemePaletteTab />}
            {activeTab === 'MOD_APPS' && <ModApplicationsTab />}
          ...
          <footer className="w-full border-t border-white/5 bg-zinc-900/20 py-6 mt-12 rounded-3xl">
            <div className="px-4 text-center space-y-2 w-full">
              ...
              <p className="text-zinc-600 text-[10px] font-mono uppercase tracking-widest">
                Guild-Master Console • V2.5 • {user?.email || 'Admin'}
              </p>
            </div>
          </footer>
        </main>
...
const OverviewTab = memo(({ changeTab }: { changeTab: (t: Tab) => void }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <AnalyticsSection accentColor="red" />
                ...
                <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1"><Badge variant="secondary" className="text-[10px] bg-white/5 hover:bg-white/10 text-zinc-400 border-none rounded-full">{user.role?.toUpperCase() || 'USER'}</Badge>{user.is_banned && <Badge variant="destructive" className="text-[10px] h-5 rounded-full">EXILED</Badge>}</div>
                        </div>
                    </div>
