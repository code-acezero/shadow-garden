...
const AnimePlayer = forwardRef<AnimePlayerRef, AnimePlayerProps>(({ 
  ...
  onEnded, onNext, onPlay, onSkipIntro, onProgress, onSeek, onInteract, onPause, onBuffer, 
  ...
  useEffect(() => {
    ...
    hasSeekedStartTimeRef.current = false;
    setIsBuffering(true); 
    ...
    if (!shouldUseNative && Hls.isSupported()) {
        ...
        if (autoPlay) {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => setIsPlaying(true)).catch((err) => {
                    console.warn("Autoplay policy restriction, attempting muted play", err);
                    video.muted = true;
                    setIsMuted(true);
      ...
      hls.on(Hls.Events.ERROR, (e, data) => {
          if (data.fatal) {
              ...
              else if (iframeUrl) setHlsFailed(true);
          }
      });
      hlsRef.current = hls;
      ...
      nativeLoadedHandler = () => { 
          ...
          if (autoPlay) {
              ...
              if (playPromise !== undefined) {
                  playPromise.then(() => setIsPlaying(true)).catch((err) => {
                      console.warn("Native autoplay restricted, trying muted play", err);
                      video.muted = true;
                      setIsMuted(true);
                      video.play().then(() => setIsPlaying(true)).catch(() => setIsPlaying(false));
                  });
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
      ...
      if (target.closest('button') || target.closest('input') || target.closest('[role="menu"]') || target.closest('.absolute.bottom-12') || target.closest('.absolute.bottom-16') || target.closest('[class*="group/seek"]') || target.closest('[class*="bg-black/60"]')) return;
  ...
  useEffect(() => {
      if (startTime > 0 && videoRef.current && !hasSeekedStartTimeRef.current) {
          try {
        ...
        <div className="flex items-center justify-between pointer-events-auto">
           ...
           <div className="flex items-center gap-2 md:gap-5 relative">
              <div className="relative">
                  ...
                  {activeMenu === 'main' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-44 md:w-56 max-h-[200px] sm:max-h-[240px] md:max-h-[260px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                          <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('quality')}} className="flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><Settings size={12} className="md:w-[13px]"/> Quality</div><span className="text-zinc-400 text-[8px] md:text-[9px] truncate ml-2 w-full">{currentQuality === -1 ? autoResolutionText : `${qualities.find(q => q.index === currentQuality)?.height}p`}</span></button>
                          <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('speed')}} className="flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><Gauge size={12} className="md:w-[13px]"/> Speed</div><span className="text-zinc-400 text-[8px] md:text-[9px]">{speed}x</span></button>
                          <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('gestures')}} className="flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><MousePointerClick size={12} className="md:w-[13px]"/> Gestures</div><span className="text-zinc-400 uppercase text-[8px] md:text-[9px]">{doubleTapMode}</span></button>
                          {castAvailable && (
                             <button onClick={(e)=>{e.stopPropagation(); handleCast(); setActiveMenu('none')}} className="lg:hidden flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><Cast size={12} className="md:w-[13px]"/> Cast</div></button>
                          )}
                          <div className="lg:hidden border-t border-white/10 mt-0.5 pt-0.5">
                             <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('vGesture')}} className="flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><MoveVertical size={12} className="md:w-[13px]"/> V-Swipe</div><span className="text-zinc-400 text-[8px] uppercase">{verticalGesture === 'vol_bright' ? 'Vol/Bri' : verticalGesture}</span></button>
                             <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('hGesture')}} className="flex items-center justify-between w-full px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:shadow-inner text-left text-[9px] md:text-[10px] font-bold transition-all"><div className="flex items-center gap-1.5"><MoveHorizontal size={12} className="md:w-[13px]"/> H-Swipe</div><span className="text-zinc-400 text-[8px] md:text-[9px] uppercase">{horizontalGesture === 'nav' ? 'Next/Prev' : horizontalGesture}</span></button>
                          </div>
                       </div>
                   )}
                   {activeMenu === 'vGesture' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-40 md:w-48 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                           <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('main')}} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 font-black text-zinc-400 border-b border-white/10 mb-0.5"><ChevronLeft size={12}/> BACK</button>
                           {['vol_bright', 'fullscreen', 'none'].map(m=>(<button key={m} onClick={(e)=>{e.stopPropagation(); updateLocalPrefs({verticalGesture: m}); setActiveMenu('main')}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold uppercase transition-all", verticalGesture===m?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>{m === 'vol_bright' ? 'Vol/Bri' : m}</button>))}
                       </div>
                   )}
                   {activeMenu === 'hGesture' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-40 md:w-48 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                           <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('main')}} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 font-black text-zinc-400 border-b border-white/10 mb-0.5"><ChevronLeft size={12}/> BACK</button>
                           {['seek', 'nav', 'volume', 'none'].map(m=>(<button key={m} onClick={(e)=>{e.stopPropagation(); updateLocalPrefs({horizontalGesture: m}); setActiveMenu('main')}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold uppercase transition-all", horizontalGesture===m?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>{m === 'nav' ? 'Next / Prev' : m}</button>))}
                       </div>
                   )}
                  
                   {activeMenu === 'quality' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-28 md:w-36 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                           <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('main')}} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 font-black text-zinc-400 border-b border-white/10 mb-0.5"><ChevronLeft size={12}/> BACK</button>
                           <button onClick={(e)=>{e.stopPropagation(); changeQuality(-1)}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all", currentQuality===-1?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>Auto</button>
                           {qualities.map(q=>(<button key={q.index} onClick={(e)=>{e.stopPropagation(); changeQuality(q.index)}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all", currentQuality===q.index?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>{q.height}p</button>))}
                       </div>
                   )}
                   {activeMenu === 'speed' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-24 md:w-32 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                           <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('main')}} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 font-black text-zinc-400 border-b border-white/10 mb-0.5"><ChevronLeft size={12}/> BACK</button>
                           {[0.5, 1, 1.25, 1.5, 2].map(r=>(<button key={r} onClick={(e)=>{e.stopPropagation(); changeSpeed(r)}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all", speed===r?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>{r}x</button>))}
                       </div>
                   )}
                   {activeMenu === 'gestures' && (
                       <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-40 md:w-48 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">
                           <button onClick={(e)=>{e.stopPropagation(); setActiveMenu('main')}} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 font-black text-zinc-400 border-b border-white/10 mb-0.5"><ChevronLeft size={12}/> BACK</button>
                           {['seek', 'playpause', 'fullscreen'].map(m=>(<button key={m} onClick={(e)=>{e.stopPropagation(); updateLocalPrefs({doubleTapMode: m}); setActiveMenu('none')}} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold uppercase transition-all", doubleTapMode===m?"bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50":"hover:bg-white/10 hover:shadow-inner")}>{m}</button>))}
                       </div>
                   )}
              </div>

              {audioTracks.length > 1 && (<div className="relative"><button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'audio' ? 'none' : 'audio'); }} className={cn("hover:text-primary-500 transition-colors active:scale-90 p-2 md:p-0", activeMenu === 'audio' && "text-primary-500")}><AudioWaveform size={20} className="md:w-5 md:h-5" /></button>{activeMenu === 'audio' && (<div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-40 md:w-48 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-0.5 animate-in slide-in-from-bottom-2">{audioTracks.map((t, i) => (<button key={i} onClick={(e) => { e.stopPropagation(); changeAudio(i); }} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all truncate active:scale-95", currentAudio === i ? "bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50" : "hover:bg-white/10 hover:shadow-inner text-zinc-400")}>{t.name || `Audio ${i+1}`}</button>))}</div>)}</div>)}
              ...
              {trackSubtitles.length > 0 && (
                <div className="relative">
                    ...
                    {activeMenu === 'subs' && (
                        <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-1.5 w-40 md:w-48 max-h-[180px] sm:max-h-[220px] md:max-h-[240px] overflow-hidden z-[70] flex flex-col animate-in slide-in-from-bottom-2">
                             <button onClick={(e) => { e.stopPropagation(); setActiveMenu('subSettings'); }} className="flex items-center gap-1.5 text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-black text-primary-500 hover:bg-white/5 transition-all mb-0.5 border-b border-white/10 shrink-0"><Settings size={10} className="md:w-3 md:h-3"/> CAPTION SETTINGS</button>
                             <div className="overflow-y-auto flex-1 flex flex-col gap-0.5 scrollbar-hide">
                               <button onClick={(e) => { e.stopPropagation(); changeSubtitle(-1); }} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all active:scale-95 shrink-0", currentSubtitle === -1 ? "bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50" : "hover:bg-white/10 hover:shadow-inner text-zinc-400")}>Off</button>
                               {trackSubtitles.map((t, i) => (
                                   <button key={i} onClick={(e) => { e.stopPropagation(); changeSubtitle(i); }} className={cn("text-[9px] md:text-[10px] px-2 py-1 rounded-full text-left font-bold transition-all truncate active:scale-95 shrink-0", currentSubtitle === i ? "bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-500/25 border border-primary-500/50" : "hover:bg-white/10 hover:shadow-inner text-zinc-400")}>{t.label || t.name || t.lang}</button>
                               ))}
                             </div>
                        </div>
                    )}
                    {activeMenu === 'subSettings' && (
                        <div className="absolute bottom-12 right-0 bg-zinc-950/80 backdrop-blur-3xl border border-white/5 ring-1 ring-white/10 rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.8)] p-2 w-44 md:w-56 max-h-[200px] sm:max-h-[240px] md:max-h-[260px] overflow-y-auto scrollbar-hide z-[70] flex flex-col gap-1.5 animate-in slide-in-from-bottom-2" onClick={(e) => e.stopPropagation()}>
