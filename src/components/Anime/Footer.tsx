...
export default function Footer() {
  ...
  return (
    <footer className="relative w-full bg-[#020202] border-t border-white/5 mt-12 overflow-hidden font-sans">
      ...
      {/* --- ANIMATION ONLY (Removed Font Loading) --- */}
      <style jsx global>{`
        @keyframes shimmer {
      ...
      {/* --- ATMOSPHERIC EFFECTS --- */}
      <div className="absolute bottom-0 left-0 w-full h-[150px] bg-gradient-to-t from-primary-950/20 via-transparent to-transparent pointer-events-none" />
      <div className="absolute -bottom-20 left-1/2 -translate-x-1/2 w-[300px] h-[100px] bg-primary-600/10 blur-[60px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-[900px] mx-auto px-6 py-8 relative z-10 flex flex-col items-center text-center">
        ...
        <Link href="/" className="group flex flex-col items-center gap-2 mb-6">
           <Crown size={24} className="text-primary-600 fill-red-950/50" />
           <h2 
             className={cn(
               "text-3xl md:text-4xl tracking-widest drop-shadow-[0_0_10px_rgba(220,38,38,0.5)]",
             ...
             // ✅ FIX: Use the variable provided by Next.js font loader
             style={{ fontFamily: 'var(--font-demoness)' }}
           >
             SHADOW GARDEN
           </h2>
           <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.3em] group-hover:text-primary-800 transition-colors">
        ...
        <div className="flex flex-col items-center gap-3 mb-8">
            <span className="text-[8px] font-black text-primary-800 uppercase tracking-[0.3em]">SPECIAL</span>
        ...
        <div className="flex items-center gap-3 mb-8">
            <button 
                onClick={handleShare}
                className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 hover:border-primary-600/30 transition-all active:scale-95 group"
            >
                <Share2 size={10} className="text-zinc-500 group-hover:text-primary-500 transition-colors" />
        ...
        <div className="w-full border-t border-white/5 pt-6 grid grid-cols-1 md:grid-cols-3 items-center gap-6 md:gap-4">
          ...
          <div className="flex justify-center order-1 md:order-2">
             <button 
               onClick={scrollToTop}
               className="w-8 h-8 rounded-full bg-primary-600/5 border border-primary-600/10 flex items-center justify-center text-primary-700 hover:bg-primary-600 hover:text-white hover:border-primary-600 transition-all duration-300 group shadow-[0_0_15px_rgba(220,38,38,0.1)] hover:shadow-[0_0_20px_rgba(220,38,38,0.4)]"
             >
          ...
          <div className="flex justify-center md:justify-end items-center gap-2 order-2 md:order-3">
             ...
             >
                <Sword size={12} className="text-primary-900 group-hover:text-primary-600 transition-colors rotate-45" />
                Guild Master: <span className="border-b border-primary-600/0 group-hover:border-primary-600 transition-all text-zinc-400 group-hover:text-white">Ace Zero</span>
             </Link>
          </div>
...
const SocialLink = ({ href, icon, label }: { href: string, icon: React.ReactNode, label: string }) => (
    <Link 
        href={href} 
        target="_blank"
        className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#080808] border border-white/5 text-zinc-500 hover:text-white hover:border-primary-600/40 hover:bg-primary-950/10 hover:shadow-[0_0_15px_rgba(220,38,38,0.1)] transition-all duration-300 group"
        title={label}
    >
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <button className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary-600/5 border border-primary-600/20 hover:bg-primary-600/10 hover:border-primary-600/40 transition-all active:scale-95 group">
                    <BookOpen size={10} className="text-primary-800 group-hover:text-primary-500 transition-colors" />
                    <span className="text-[9px] font-black text-primary-800 uppercase tracking-widest group-hover:text-primary-500 transition-colors">GUILD ARCHIVES</span>
                </button>
            </DialogTrigger>
            <DialogContent className="bg-[#050505] border-primary-900/30 text-white max-w-2xl w-[95vw] h-[75vh] md:h-[550px] rounded-3xl shadow-[0_0_50px_-10px_rgba(220,38,38,0.2)] p-0 overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="flex-1 flex flex-col min-h-0 bg-[#080808]">
                    <DialogHeader className="p-6 pb-2 shrink-0">
                        <DialogTitle className="text-lg font-black font-[Cinzel] text-primary-600 tracking-tighter flex items-center gap-2">
                    ...
                    <div className="p-4 border-t border-white/5 bg-black/20 flex justify-between items-center text-[9px] font-bold text-zinc-600 uppercase tracking-widest">
                        ...
                        <div className="flex items-center gap-2">
                            <span>Signed:</span>
                            <span className="text-primary-600">Ace Zero</span>
                        </div>
                    </div>
                </div>
...
const ArchiveTab = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
    <button 
        ...
        className={cn(
            ...
            active ? "bg-primary-600/10 text-primary-500 border border-primary-600/20" : "text-zinc-500 hover:text-white hover:bg-white/5"
        )}
    >
        {icon}
        {label}
...
const PurgeContent = () => (
    <div className="space-y-4 text-xs text-zinc-400 leading-relaxed">
        ...
        <div className="mt-4">
            ...
            <a href="mailto:dmca@shadowgarden.io" className="text-primary-500 hover:underline">dmca@shadowgarden.io</a>
        </div>
    </div>
);
...
const RuleItem = ({ num, text }: { num: string, text: string }) => (
    <div className="flex gap-4 items-start p-3 hover:bg-white/5 rounded-lg transition-colors border border-transparent hover:border-white/5">
        <span className="text-xs font-black text-primary-800 mt-0.5 font-[Cinzel]">{num}</span>
...
const QnAItem = ({ q, a }: { q: string, a: string }) => (
    <div className="bg-white/5 p-3 rounded-lg border border-white/5">
        <h4 className="text-[11px] font-black text-white mb-1 flex items-center gap-2">
            <span className="text-primary-600">Q:</span> {q}
        </h4>
