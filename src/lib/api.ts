...
export class AnimeAPI_V2 {
  ...
  private static async request<T>(endpoint: string, params: Record<string, any> = {}): Promise<T | null> {
    try {
      ...
      // FIX: Use URLSearchParams for correct encoding of special characters like '?'
      ...
      // Encode the entire target URL as a component for the proxy
  ...
  static async getAZList(sortOption = 'all', page = 1): Promise<V2AZListResult | null> {
  ...
  static async search(query: string, page = 1, filters = {}): Promise<V2SearchResult | null> {
