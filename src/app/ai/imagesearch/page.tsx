...
export default function ImageSearchPage() {
  ...
  const handleWatchNow = async () => {
      ...
      try {
          const searchData = await AnimeService.search(targetTitle);
          let foundAnime = null;

          if (searchData?.results) {
              foundAnime = searchData.results.find((anime: any) => {
                  const apiTitle = (anime.title || "").toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col relative overflow-hidden font-sans">
      ...
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-primary-600/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center pt-24 pb-20 px-4 z-10">
        ...
        <div className="text-center mb-8 relative">
            ...
            >
                <ScanLine className="text-primary-500 animate-pulse w-5 h-5" />
                <span className={`text-primary-500 text-sm tracking-[0.2em] font-bold ${hunters.className}`}>FIND THE SCENE</span>
                <ScanLine className="text-primary-500 animate-pulse w-5 h-5" />
            </motion.div>
            ...
            >
                VISUAL <span className="text-primary-600">ARCHIVE</span>
            </motion.h1>
            ...
            {quota && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-[10px] text-zinc-400">
                    <span className={quota.used >= quota.total ? "text-primary-500" : "text-green-500"}>●</span>
        ...
        <motion.div layout className="w-full max-w-2xl relative">
            <AnimatePresence mode="wait">
                ...
                {!selectedImage && (
                    <motion.div
                        ...
                        className={cn(
                            ...
                            isDragging ? "border-primary-500 bg-primary-500/5 scale-[1.02]" : "border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/10"
                        )}
                    >
                        ...
                        <div className="p-4 rounded-full bg-black/40 border border-white/10 mb-6 group-hover:scale-110 transition-transform duration-300 shadow-2xl relative">
                            <Upload size={32} className="text-zinc-400 group-hover:text-primary-500 transition-colors" />
                        ...
                        <div className="absolute top-0 left-0 w-12 h-12 md:w-16 md:h-16 border-t-2 border-l-2 border-white/10 rounded-tl-[2rem] pointer-events-none transition-colors group-hover:border-primary-500/30" />
                        <div className="absolute bottom-0 right-0 w-12 h-12 md:w-16 md:h-16 border-b-2 border-r-2 border-white/10 rounded-br-[2rem] pointer-events-none transition-colors group-hover:border-primary-500/30" />
                    </motion.div>
                )}
                ...
                {selectedImage && (
                    ...
                    >
                        ...
                        {isScanning && (
                            <div className="absolute inset-0 pointer-events-none z-20">
                                <motion.div 
                                    ...
                                    className="absolute left-0 right-0 h-0.5 bg-primary-500 shadow-[0_0_20px_2px_rgba(220,38,38,0.8)]"
                                />
                                ...
                                <div className="absolute bottom-6 left-6 flex items-center gap-3 bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
                                    ...
                                    <span className={`text-primary-500 text-sm tracking-widest ${hunters.className}`}>ANALYZING...</span>
                                </div>
                            </div>
                        )}
                                ...
                                <div className="p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="flex items-center gap-3 text-xs sm:text-sm font-mono mt-2">
                                            {/* Capsule EP Badge */}
                                            <span className="bg-primary-600 px-3 py-1 rounded-full text-white font-bold text-xs shadow-lg shadow-primary-600/20">
                                    ...
                                    <Button 
                                        ...
                                        className="w-full md:w-auto bg-gradient-to-r from-primary-600 to-primary-800 hover:from-primary-500 hover:to-primary-700 text-white rounded-full px-6 shadow-[0_0_15px_rgba(220,38,38,0.4)] shrink-0 h-10 md:h-12 text-sm font-bold tracking-wide"
                                    >
                                        ...
                                        >
                                            <div className="p-4 grid grid-cols-2 gap-y-4 gap-x-8 text-xs text-zinc-400">
                                                ...
                                                <div>
                                                    ...
                                                    <span className={scanResult.anilist.isAdult ? "text-primary-500 font-bold" : "text-green-500"}>
                        ...
                        <button onClick={clearImage} className="absolute top-4 right-4 p-2 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-primary-600 hover:text-white transition-colors border border-white/10 z-50">
                            <X size={20} />
                        </button>
                    </motion.div>
                )}
            ...
            >
                ...
                <div className="col-span-2 md:col-span-1 bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-3 backdrop-blur-sm">
                    <XCircle className="text-primary-500 shrink-0 mt-0.5" size={18} />
                    <div>
                ...
                <div className="col-span-2 bg-primary-600/5 border border-primary-500/10 rounded-xl p-3 flex items-center justify-center gap-2 text-center">
                    <HelpCircle className="text-primary-500" size={14} />
                    <p className="text-primary-400/80 text-[10px] font-mono uppercase">System powered by trace.moe • Indexing 30,000+ hours of anime</p>
                </div>
            </motion.div>
