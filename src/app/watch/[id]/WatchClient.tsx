...
function WatchContent() {
    ...
    const handleEpisodeClick = useCallback((id: string) => {
        ...
        // Trigger episode switch ad (debounced)
    ...
    const handleCreateWatchRoom = async () => {
        ...
        try {
            ...
            const { data: room, error: roomError } = await supabase
                ...
                .insert({
                    code,
                    title: anime?.title ? `${anime.title} Watch Room` : 'Anime Watch Room',
                    host_id: user.id,
                    media_id: animeId,
                    media_type: 'anime',
      ...
      `}</style>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className={cn("w-full flex flex-col items-center relative px-4 pt-0 mt-0 z-10")}>
                ...
                <div className="w-full flex flex-col xl:grid xl:grid-cols-12 gap-8 xl:items-stretch">
                    <div className="xl:col-span-8 w-full flex flex-col justify-between gap-2 order-1">
                        <div 
                            ...
                            className={cn(
                                ...
                                settings.dimMode ? "z-[60] ring-2 ring-primary-500/50 shadow-[0_0_80px_rgba(0,0,0,0.9)]" : "z-10"
                            )} 
                            ...
                            {streamUrl ? (
                                streamUrl.includes('.m3u8') || streamUrl.includes('.mp4') || streamUrl.includes('/api/proxy') ? (
                                    <AnimePlayer 
                                        ...
                                        onSeek={(seekTime: number) => { progressRef.current = seekTime; saveProgress(true); }} 
                                        ...
                                        onError={(err: any) => { console.warn("Player Error, forcing reload..."); failedServersRef.current.push(settings.server.toLowerCase()); setFetchTrigger((prev: number) => prev + 1); }} 
                                    />
                                ) : (
                            ...
                            <div className={cn("hidden lg:flex w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-[30px] shadow-[0_8px_32px_0_rgba(0,0,0,0.2)] px-5 py-3 flex-col gap-2 overflow-visible mt-3 transition-all duration-500", settings.dimMode ? "z-[96] relative" : "relative z-10")}>
                                ...
                                <div className="flex w-full justify-between items-center gap-3 border-t border-white/5 pt-3">
                                    ...
                                    <div className="flex items-center gap-3 flex-shrink-0">
                                        ...
                                        <button onClick={() => setShowShareModal(true)} className="flex items-center justify-center gap-1.5 px-3 h-8 rounded-full border border-white/10 bg-white/5 text-zinc-300 text-[10px] font-black uppercase tracking-widest transition-all hover:bg-primary-600 hover:border-primary-500 hover:text-white whitespace-nowrap shadow-md" title="Share Episode">
                                            <Share2 size={12} /> Share
                                        </button>
                                        <button onClick={handleCreateWatchRoom} className="flex items-center justify-center gap-1.5 px-3 h-8 rounded-full border border-white/10 bg-white/5 text-zinc-300 text-[10px] font-black uppercase tracking-widest transition-all hover:bg-primary-600 hover:border-primary-500 hover:text-white whitespace-nowrap shadow-md cursor-pointer" title="Create Watch Room">
                    ...
                    <div className="xl:col-span-4 w-full h-[500px] xl:h-auto xl:relative xl:self-stretch order-5">
            ...
            <PostShareModal
                ...
                customTitle={`${anime?.title || 'Anime'} - Episode ${currentEpisode?.number || '1'}`}
                customContent={`Watch ${anime?.title || 'Anime'} Episode ${currentEpisode?.number || '1'} on Shadow Garden!`}
            />
        </div>
    );
}
