...
const useWatchSettings = () => {
  const { user } = useAuth();
  // [ADDED] autoNext to defaults
  const [settings, setSettings] = useState({ autoPlay: true, autoNext: true, autoSkip: false, dimMode: false, server: 'hd-1', category: 'sub' as 'sub' | 'dub' | 'raw', volume: 1, speed: 1, quality: -1 });
...
function WatchContent() {
  ...
  // Resume & Tracking
  const [resumeTime, setResumeTime] = useState(0); // Holds the DB value
  const [isResumeLoaded, setIsResumeLoaded] = useState(false); // [CRITICAL] Gates player rendering
  const [watchedEpNumbers, setWatchedEpNumbers] = useState<number[]>([]);
  const progressRef = useRef(0); const lastSavedRef = useRef(0);
  ...
  const flushData = useCallback(() => {
    ...
    if (!entry) return;
    const payload = { user_id: user?.id, anime_id: entry.animeId, episode_id: entry.episodeId, episode_number: entry.episodeNumber, progress: entry.progress, last_updated: new Date().toISOString() };
    ...
    if (supabaseUrl && supabaseKey && user) {
        fetch(`${supabaseUrl}/rest/v1/user_continue_watching`, { method: 'POST', headers: { 'apikey': supabaseKey, 'Authorization': `Bearer ${authTokenRef.current}`, 'Content-Type': 'application/json', 'Prefer': 'resolution=merge-duplicates' }, body: JSON.stringify(payload), keepalive: true }).catch(err => console.error("Beacon Failed", err));
    }
  }, [animeId, user]);
  ...
  const saveProgress = useCallback(async (force = false) => {
      ...
      // Only read from Ref (Pushed by Player)
      const cleanSeconds = Math.floor(progressRef.current);

      // Guard: Don't save 0:00 unless strictly forced (like video ended)
      if (cleanSeconds === 0 && !force) return;
      if (!force && Math.abs(cleanSeconds - lastSavedRef.current) < 5) return;
      
      const currentEpObj = episodes.find(e => e.episodeId === currentEpId);
      let epNum = 0;
      if (currentEpObj) { if (typeof currentEpObj.number === 'number') epNum = currentEpObj.number; else if (typeof currentEpObj.number === 'string') epNum = parseFloat(currentEpObj.number); }
      ...
      if (user && supabase) {
         try {
             await (supabase.from('user_continue_watching') as any).upsert({ user_id: user.id, anime_id: animeId, episode_id: currentEpId, episode_number: epNum, episode_image: epImage, total_episodes: episodes.length, type: info.anime.stats.type, progress: cleanSeconds, last_updated: new Date().toISOString() });
             
             // [ADDED] Watched History Logic
             ...
             if (cleanSeconds > (durationSec * 0.80) || (cleanSeconds > 1200 && force)) { 
                  await (supabase.from('user_watched_history') as any).upsert({ user_id: user.id, anime_id: animeId, episode_number: epNum }, { onConflict: 'user_id, anime_id, episode_number' });
                  if (!watchedEpNumbers.includes(epNum)) setWatchedEpNumbers(prev => [...prev, epNum]);
             }
         } catch (err) {}
      }
  }, [animeId, currentEpId, episodes, info, user, watchedEpNumbers]);

  const handleManualSeek = () => { setTimeout(() => saveProgress(true), 1000); };
  const handlePlayerProgress = (state: any) => { 
      const played = state.playedSeconds ?? state.target?.currentTime ?? state; 
      if (typeof played === 'number') progressRef.current = played;
  };

  // 4. LOAD & RESUME (GATEKEEPER)
  useEffect(() => {
    // We only run this when 'currentEpId' changes.
    // We DISABLE the player (isResumeLoaded = false) until we fetch the data.
    if (!currentEpId) return;

    setIsResumeLoaded(false); // [CRITICAL] Block Player
    setStreamUrl(null); // Clear old stream

    const prepareEpisode = async () => {
        // Fetch Resume
        let time = 0;
        if (user) {
            const { data } = await (supabase.from('user_continue_watching') as any).select('progress').eq('user_id', user.id).eq('anime_id', animeId).eq('episode_id', currentEpId).maybeSingle();
        ...
        setResumeTime(time);
        progressRef.current = time; // Seed ref
        setIsResumeLoaded(true); // [CRITICAL] Unlock Player

        // [ADDED] Instant History Mark
        if (user && episodes.length > 0) {
            const ep = episodes.find(e => e.episodeId === currentEpId);
            if (ep) {
                let epNum = parseFloat(String(ep.number));
                if (!isNaN(epNum)) {
                   try { await (supabase!.from('user_watched_history') as any).upsert({ user_id: user.id, anime_id: animeId, episode_number: epNum }, { onConflict: 'user_id, anime_id, episode_number' }); setWatchedEpNumbers(prev => prev.includes(epNum) ? prev : [...prev, epNum]); } catch(e){}
                }
            }
        }
    };
    prepareEpisode();
  // [FIX] Changed 'user' to 'user?.id' to avoid re-running on tab switch
  }, [currentEpId, user?.id, animeId, episodes]);

  // 5. STREAM FETCH
  useEffect(() => {
      // Only fetch stream AFTER Resume is ready
      if (!isResumeLoaded || !currentEpId) return;
      
      let isMounted = true; const controller = new AbortController();
      const loadStream = async () => {
        try {
          setIsStreamLoading(true); setStreamError(null);
  ...
  }, [isResumeLoaded, currentEpId, settings.category, settings.server]); 
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        hybridInfo.anime.stats.malScore = v2Data.anime.moreInfo.malscore || v2Data.anime.info.stats.malScore || v2Data.anime.moreInfo.mal_score || '?';
  ...
  // [FIX] Changed 'user' to 'user?.id' here too to prevent tab switch reloads
  }, [animeId, urlEpId, user?.id]);
  ...
  const handleEpisodeClick = (id: string) => { 
      ...
      setCurrentEpId(id); 
      // [FIX] Update URL explicit
      window.history.replaceState(null, '', `/watch/${animeId}?ep=${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-100 pb-20 pt-24 relative font-sans overflow-x-hidden">
      {/* IMMERSIVE MODE */}
      <style jsx global>{`::-webkit-scrollbar { display: none; } * { scrollbar-width: none; } ${hideInterface ? `nav, header, footer, .bottom-navigation, div[class*="navbar"], div[class*="header"], div[class*="footer"], .fixed.top-0, .fixed.bottom-0 { opacity: 0 !important; pointer-events: none !important; transition: opacity 0.5s ease-in-out; }` : ''}`}</style>
      ...
      <div className="w-full relative z-40 flex justify-center bg-[#050505]">
        <div className="w-full max-w-[1400px] px-4 md:px-8 mt-6">
            <div className="w-full aspect-video bg-black rounded-3xl overflow-hidden border border-white/5 shadow-2xl relative shadow-red-900/10">
                {isStreamLoading || !isResumeLoaded ? <FantasyLoader text="CHANNELING..." /> : 
                 ...
                 streamUrl ? <AnimePlayer 
                    key={`${currentEpId}-${streamUrl}-${resumeTime}`} 
                    ref={playerRef} 
                    url={streamUrl} 
                    subtitles={subtitles} 
                    ...
                    // [ADDED] Auto Next
                    onEnded={() => { saveProgress(true); if (settings.autoNext && nextEpisode) handleEpisodeClick(nextEpisode.episodeId); }}
                    onInteract={() => setTimeout(() => saveProgress(true), 1000)}
      ...
      {/* CONTROLS */}
      <div className="w-full flex justify-center bg-[#0a0a0a] border-b border-white/5 relative z-40 shadow-red-900/10 shadow-lg"><div className="w-full max-w-[1400px] px-4 md:px-8 py-3 flex flex-col lg:flex-row gap-4 justify-between items-center"><div className="flex-1 min-w-0 flex items-center gap-4 w-full sm:w-auto overflow-hidden"><MarqueeTitle text={currentEpisode?.title || `Episode ${currentEpisode?.number}`} /><div className="hidden sm:block"><NextEpisodeTimer schedule={nextEpSchedule} status={anime.moreInfo.status} /></div><WatchListButton animeId={anime.id} animeTitle={anime.name} animeImage={anime.poster} currentEp={currentEpisode?.number} /></div>
