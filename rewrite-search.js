...
import {
  ...
  Calendar, Layers, Info, CheckCircle, RotateCcw, ArrowDownAZ, Hash, PlaySquare
} from 'lucide-react';
...
const getThemeStyles = (lib: string) => {
  switch (lib) {
    case 'donghua': return { 
        bgActive: 'bg-orange-600', shadowActive: 'shadow-orange-600/20', textActive: 'text-orange-400', 
        borderActive: 'border-orange-500', bgHover: 'hover:bg-orange-600/20', textHover: 'group-hover:text-orange-300',
        textMain: 'text-orange-500', bgLight: 'bg-orange-600/20', borderLight: 'border-orange-500/30'
    };
    case 'hindi': return { 
        bgActive: 'bg-purple-600', shadowActive: 'shadow-purple-600/20', textActive: 'text-purple-400', 
        borderActive: 'border-purple-500', bgHover: 'hover:bg-purple-600/20', textHover: 'group-hover:text-purple-300',
        textMain: 'text-purple-500', bgLight: 'bg-purple-600/20', borderLight: 'border-purple-500/30'
    };
    case 'drama': return { 
        bgActive: 'bg-cyan-600', shadowActive: 'shadow-cyan-600/20', textActive: 'text-cyan-400', 
        borderActive: 'border-cyan-500', bgHover: 'hover:bg-cyan-600/20', textHover: 'group-hover:text-cyan-300',
        textMain: 'text-cyan-500', bgLight: 'bg-cyan-600/20', borderLight: 'border-cyan-500/30'
    };
    case 'movies': return { 
        bgActive: 'bg-emerald-600', shadowActive: 'shadow-emerald-600/20', textActive: 'text-emerald-400', 
        borderActive: 'border-emerald-500', bgHover: 'hover:bg-emerald-600/20', textHover: 'group-hover:text-emerald-300',
        textMain: 'text-emerald-500', bgLight: 'bg-emerald-600/20', borderLight: 'border-emerald-500/30'
    };
    default: return { 
        bgActive: 'bg-primary-600', shadowActive: 'shadow-primary-600/20', textActive: 'text-primary-400', 
        borderActive: 'border-primary-500', bgHover: 'hover:bg-primary-600/20', textHover: 'group-hover:text-primary-300',
        textMain: 'text-primary-500', bgLight: 'bg-primary-600/20', borderLight: 'border-primary-500/30'
    };
  }
};

// --- SUB COMPONENTS ---
function FilterChip({ label, active, onClick, themeStyles }: { label: string; active: boolean; onClick: () => void; themeStyles: any }) {
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 ${
        active
          ? `${themeStyles.bgActive} ${themeStyles.borderActive} text-white shadow-lg ${themeStyles.shadowActive}`
...
function SelectDropdown({ label, icon: Icon, options, value, onChange, themeStyles }: any) {
              ...
              {options.map((opt: string) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs rounded-lg transition-colors flex justify-between items-center uppercase ${
                    value === opt ? `${themeStyles.bgLight} ${themeStyles.textActive}` : 'text-zinc-300 hover:bg-white/10'
                  }`}
                >
                  {opt.replace(/-/g, ' ')}
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <div className="relative pt-20 pb-8 px-4 overflow-hidden">
        <div className={`absolute inset-0 bg-gradient-to-b ${themeStyles.bgLight} to-transparent opacity-30`} />
        <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] ${themeStyles.bgActive} rounded-full blur-[150px] opacity-20`} />
        
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex items-center gap-3 mb-2">
            <div className={`h-px w-8 ${themeStyles.bgActive}`} />
            <span className={`${themeStyles.textMain} text-[10px] tracking-[0.3em] font-bold uppercase font-lemon`}>
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase transition-all ${
                      selectedLibrary === lib.id 
                        ? `${getThemeStyles(lib.id).bgActive} text-white shadow-lg ${getThemeStyles(lib.id).shadowActive}` 
          ...
          <form onSubmit={handleSearch} className="mt-6 flex gap-3">
            <div className="relative flex-1 max-w-2xl">
              ...
              <input
                ...
                placeholder={`Search ${selectedLibrary}...`}
                className={`w-full bg-white/5 border border-white/10 rounded-2xl py-3.5 pl-12 pr-4 text-white text-sm focus:outline-none focus:${themeStyles.borderActive}/50 focus:bg-white/[0.08] transition-all placeholder-zinc-500`}
              />
              {query && (
            ...
            <button
              type="submit"
              className={`px-6 py-3.5 ${themeStyles.bgActive} rounded-2xl text-sm font-bold transition-colors shadow-lg ${themeStyles.shadowActive} text-white`}
            >
              Search
            </button>
            <button
              ...
              className={`px-4 py-3.5 rounded-2xl border text-sm font-bold transition-all ${
                showFilters ? `${themeStyles.bgLight} ${themeStyles.borderLight} ${themeStyles.textActive}` : 'bg-white/5 border-white/10 text-zinc-400 hover:text-white hover:bg-white/10'
              }`}
            >
            ...
            <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
              ...
              <div>
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-3 block flex items-center gap-2">
                  <Hash size={12} /> {selectedLibrary === 'drama' || selectedLibrary === 'movies' ? 'Categories' : 'Genres'}
                </span>
                      ...
                      <FilterChip
                        ...
                        onClick={() => {
                          if (selectedLibrary === 'drama' || selectedLibrary === 'movies') {
                            // Drama and Movies often only support single category select via API params
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {selectedLibrary !== 'donghua' && selectedLibrary !== 'drama' && selectedLibrary !== 'movies' && <SelectDropdown label="Type" icon={Tv} options={selectedLibrary === 'hindi' ? HINDI_TYPES : TYPES} value={selectedType} onChange={setSelectedType} themeStyles={themeStyles} />}
                {selectedLibrary !== 'drama' && selectedLibrary !== 'movies' && <SelectDropdown label="Status" icon={Info} options={selectedLibrary === 'donghua' ? DONGHUA_STATUS : selectedLibrary === 'hindi' ? HINDI_STATUS : STATUS_OPTIONS} value={selectedStatus} onChange={setSelectedStatus} themeStyles={themeStyles} />}
                {selectedLibrary === 'main' && <SelectDropdown label="Season" icon={Calendar} options={SEASONS} value={selectedSeason} onChange={setSelectedSeason} themeStyles={themeStyles} />}
                {selectedLibrary === 'main' && <SelectDropdown label="Year" icon={Layers} options={YEARS} value={selectedYear} onChange={setSelectedYear} themeStyles={themeStyles} />}
              </div>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-white/5">
                ...
                <button
                  ...
                  className={`px-8 py-2.5 ${themeStyles.bgActive} rounded-xl text-sm font-bold transition-colors shadow-lg ${themeStyles.shadowActive} text-white`}
                >
                  Apply Filters
                </button>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          <div className="h-[50vh] flex flex-col items-center justify-center gap-4">
            <Loader2 className={`w-10 h-10 animate-spin ${themeStyles.textMain}`} />
          ...
          <div className="h-[50vh] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-zinc-500 text-sm text-center max-w-md">
              {keyword ? `We couldn't find anything matching "${keyword}". Try different keywords or filters.` : 'Try searching or applying some filters.'}
            </p>
          ...
          <>
            ...
            {(selectedGenres.length > 0 || selectedType || selectedStatus) && (
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedGenres.map(g => (
                  <span key={g} className={`flex items-center gap-1.5 px-3 py-1 rounded-full ${themeStyles.bgLight} ${themeStyles.borderLight} ${themeStyles.textActive} text-xs font-bold`}>
                    {g}
                    <button onClick={() => toggleGenre(g)} className="hover:text-white"><X size={12} /></button>
                  </span>
                ))}
                {selectedType && (
                  <span className={`flex items-center gap-1.5 px-3 py-1 rounded-full ${themeStyles.bgLight} ${themeStyles.borderLight} ${themeStyles.textActive} text-xs font-bold uppercase`}>
                    {selectedType}
            ...
            <div className={viewMode === 'grid'
              ...
              {results.map((item: any) => {
                if (selectedLibrary === 'drama' || selectedLibrary === 'movies') {
                    // Use MediaCard for Drama and Movies
                    return <MediaCard key={item.id} item={item} theme={selectedLibrary === 'drama' ? 'cyan' : 'emerald'} />;
                } else {
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border text-sm font-bold transition-all ${
                  currentPage === 1
                    ...
                    : `border-white/10 text-zinc-300 ${themeStyles.hoverBg} ${themeStyles.borderActive} hover:text-white`
                }`}
              >
                ...
                typeof p === 'string' ? (
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl text-sm font-bold transition-all ${
                      p === currentPage
                        ? `${themeStyles.bgActive} text-white shadow-lg ${themeStyles.shadowActive} ${themeStyles.borderActive}`
